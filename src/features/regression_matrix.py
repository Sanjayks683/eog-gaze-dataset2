...
==================================
Regression feature matrices built straight from the raw recordings, for the scripts
that need several preprocessing variants (nested cross-validation, known-start
fusion) without writing main.py's processed-data files each time.

It runs the pipeline's own preprocess -> window -> feature code under the current
CFG settings, so a matrix holds the same inputs `python main.py --phase train` uses
for that config: engineered window features, then the context features split into
the baseline/lag block and the rolling-range block.
"""
...
def load_unified_trials() -> List[Trial]:
    """Raw Dataset 2 recordings with bipolar H and V added."""
    return [to_common_schema(t) for t in load_trials()]
