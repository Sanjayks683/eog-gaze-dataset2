...
Estimators (all see only the EOG and the starting gaze; A is a 2-channel H/V linear
map without intercept, as in Barbara et al., BSPC 47, 2019):
  ...
  * fused (given per-sample absolute gaze estimates from a cross-subject model, see
    scripts/known_start_fusion.py): the detected-saccade estimate plus a causal
    first-order low-pass, time constant tau, of the absolute estimate minus it,
    starting from 0 at the known start. The saccade sum supplies fast changes and the
    absolute model the slow level, so errors that build up over a segment are pulled
    back. tau is chosen per axis from FUSION_TAUS_SEC on the fit data (inf = no fusion).
"""
...
def ground_truth_labels(hv: np.ndarray, fs: float, control: np.ndarray) -> np.ndarray:
    ...
    blink threshold is gt_blink_ratio x the median largest |V difference| inside the
    protocol's blink intervals.
    """
    cfg = CFG.known_start
...
def _estimators(d: Dict) -> Tuple[str, ...]:
    return ("saccades", "level") + (("fused",) if d.get("absolute") is not None else ())
...
def _fit_estimators(pairs: Dict[str, tuple], d: Dict, fit_segments: Optional[List[tuple]] = None,
    ...
    """
    A per estimator. With absolute estimates, "fused" gets the detected-saccade A and
    per-axis time constants chosen on fit_segments, a list of (recording, windows,
    gain); the choice is appended to tau_log.
    """
...
def _protocol_stats(data: List[Dict]) -> Dict[str, float]:
    ...
    rows = []
    for d in data:
        labels, windows = d["labels"], d["windows"]
...
def run_known_start_protocol(trials: List[Trial], absolute: Optional[Dict[tuple, np.ndarray]] = None) -> Dict:
    ...
    if not has_absolute:  # all recordings or none, so every subject has the same rows
        for d in data:
            d["absolute"] = None
...
# Published Dataset 2 known-start results, Barbara et al., BSPC 86 (2023), Tables 2-3:
# (method, segments, fixation MAE H, V, excluded outlier share).
PAPER_KNOWN_START = [
