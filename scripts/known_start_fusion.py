...
=============================
Known-start estimation fused with the cross-subject absolute gaze model.
...
The absolute model is the real-time XGBoost of configs/realtime_weighted.yaml
(past-only 60 s robust line, context + range features, weighted training), trained
5-fold cross-subject, so a subject's estimates never use that subject's data. Its
window predictions become a per-sample series, each held from the end of its window
(causal).

    python scripts/known_start_fusion.py

Results: reports/known_start_fusion/ (known_start_protocol.json, known_start_table.csv,
absolute_model.json). Features are cached in data/processed_nested/realtime_60s.npz
(shared with scripts/nested_cv.py).
"""
...
def feature_matrix(trials) -> dict:
    pp = CFG.preprocessing
    assert (pp.drift_removal_method, pp.baseline_window_sec, pp.baseline_causal) == ("robust_line", 60.0, True)
    path = os.path.join(CFG.paths.project_root, "data", "processed_nested", "realtime_60s.npz")
...
def main() -> None:
    ...
    load_config_from_yaml(os.path.join(root, "configs", "realtime_weighted.yaml"))
    CFG.cv.xgb_device = args.xgb_device
    trials = load_unified_trials()
    matrix = feature_matrix(trials)
    ...
    for train_idx, test_idx in get_folds(subjects):
        p, _, kept = fit_predict_regression_fold(X, y, train_idx, test_idx, "xgb", fixation,
    ...
    out_dir = os.path.join(root, "reports", "known_start_fusion")
    ...
    with open(os.path.join(out_dir, "absolute_model.json"), "w") as f:
        json.dump({"description": "5-fold cross-subject real-time XGBoost (context + range features, weighted "
                   ...
                   "config": "configs/realtime_weighted.yaml", "xgb_device": CFG.cv.xgb_device,
                   **absolute}, f, indent=2)
