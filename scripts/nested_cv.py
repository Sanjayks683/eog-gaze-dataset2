...
=====================
Nested cross-subject evaluation of the pipeline settings (XGBoost).

The settings in configs/ were chosen by comparing variants on the same 5 test folds
the results report. Here the outer loop is the pipeline's 5-fold GroupKFold, and
inside each outer fold every candidate is scored by a 4-fold GroupKFold over the
outer-training subjects only. The best candidate by the inner criterion is refit on
all outer-training subjects and scored once on the held-out subjects. Every candidate
is also scored directly on the outer test folds ("fixed" results, the way the
config results are reported), so the gap between the two shows how optimistic
choosing on the test folds was.
...
The config supplies the channels and context/range settings; the track sets the
baseline method (realtime: past-only robust line, offline: centred robust mean).
Results: reports/nested_cv/<track>/; feature caches and predictions:
data/processed_nested/ (git-ignored). A full run takes about an hour per track on a
GPU (--xgb-device cuda).
"""
...
def main() -> None:
    ...
    out_dir = os.path.join(root, "reports", "nested_cv", args.track)
    cache_dir = os.path.join(root, "data", "processed_nested")
    ...
    header = {
        ...
        "dataset": "dataset2", "track": args.track, "config": args.config, "xgb_device": CFG.cv.xgb_device,
